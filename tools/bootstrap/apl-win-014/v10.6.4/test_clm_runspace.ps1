...
Invoke-InConstrainedLanguage -Description 'Resolve-ClmPolicyEvidence (braced expected, unbraced live)' -Script ". '$helpers'; `$fake = [ordered]@{ OperationResult=0; Policies=@([ordered]@{ PolicyID='dc1c604c-46ea-40b7-9f47-cf582b225d5e'; BasePolicyID='dc1c604c-46ea-40b7-9f47-cf582b225d5e'; FriendlyName='Arvectum APL-WIN-014 Lab Base'; IsOnDisk=`$true; IsEnforced=`$true; IsAuthorized=`$true; PolicyOptions=@('Enabled:Allow Supplemental Policies') }, [ordered]@{ PolicyID='8d593266-9e22-463f-b594-df9b734d02de'; BasePolicyID='dc1c604c-46ea-40b7-9f47-cf582b225d5e'; FriendlyName='Arvectum APL-WIN-014 Harness V10.6.4 Bootstrap'; Version='10.0.0.17'; IsOnDisk=`$true; IsEnforced=`$true; IsAuthorized=`$true; PolicyOptions=@() }) }; Resolve-ClmPolicyEvidence -CiToolResult `$fake -ExpectedBasePolicyId '{DC1C604C-46EA-40B7-9F47-CF582B225D5E}' -ExpectedBaseFriendlyName 'Arvectum APL-WIN-014 Lab Base' -ExpectedBootstrapPolicyId '{8D593266-9E22-463F-B594-DF9B734D02DE}' -ExpectedBootstrapFriendlyName 'Arvectum APL-WIN-014 Harness V10.6.4 Bootstrap' | Out-Null"
Invoke-InConstrainedLanguage -Description 'Resolve-ClmPolicyEvidence (unbraced expected, braced live)' -Script ". '$helpers'; `$fake = [ordered]@{ OperationResult=0; Policies=@([ordered]@{ PolicyID='{DC1C604C-46EA-40B7-9F47-CF582B225D5E}'; BasePolicyID='{DC1C604C-46EA-40B7-9F47-CF582B225D5E}'; FriendlyName='Arvectum APL-WIN-014 Lab Base'; IsOnDisk=`$true; IsEnforced=`$true; IsAuthorized=`$true; PolicyOptions=@('Enabled:Allow Supplemental Policies') }, [ordered]@{ PolicyID='{8D593266-9E22-463F-B594-DF9B734D02DE}'; BasePolicyID='{DC1C604C-46EA-40B7-9F47-CF582B225D5E}'; FriendlyName='Arvectum APL-WIN-014 Harness V10.6.4 Bootstrap'; Version='10.0.0.17'; IsOnDisk=`$true; IsEnforced=`$true; IsAuthorized=`$true; PolicyOptions=@() }) }; Resolve-ClmPolicyEvidence -CiToolResult `$fake -ExpectedBasePolicyId 'dc1c604c-46ea-40b7-9f47-cf582b225d5e' -ExpectedBaseFriendlyName 'Arvectum APL-WIN-014 Lab Base' -ExpectedBootstrapPolicyId '8d593266-9e22-463f-b594-df9b734d02de' -ExpectedBootstrapFriendlyName 'Arvectum APL-WIN-014 Harness V10.6.4 Bootstrap' | Out-Null"
$malformedGuidFailed = $false
try { Invoke-InConstrainedLanguage -Description 'Convert-ClmPolicyGuidIdentity (malformed)' -Script ". '$helpers'; Convert-ClmPolicyGuidIdentity '{8D593266-9E22-463F-B594-DF9B734D02DE' | Out-Null" } catch { $malformedGuidFailed = $true }
if (-not $malformedGuidFailed) { throw 'Malformed policy GUID should fail in CLM.' }
Write-Host '  PASS: Convert-ClmPolicyGuidIdentity (malformed rejected)'
