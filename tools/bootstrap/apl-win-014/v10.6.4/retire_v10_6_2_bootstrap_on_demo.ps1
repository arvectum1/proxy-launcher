...
if ($RetirePolicyId -ine $expectedRetirePolicyId) { throw "RetirePolicyId must be the canonical V10.6.2 policy: $expectedRetirePolicyId" }
...
if ($oldAuthoring.schema -ne 'arvectum.proxy.apl-win-014-v10.6.2-bootstrap-authoring.v1' -or $oldAuthoring.base_policy_id -ne $basePolicyIdText -or $oldAuthoring.supplemental_policy_id -ne $expectedRetirePolicyId -or $oldAuthoring.supplemental_policy_friendly_name -ne $expectedRetireFriendlyName -or $oldAuthoring.supplemental_policy_version -ne $expectedRetireVersion) { throw 'Authoring evidence does not match the canonical V10.6.2 policy.' }
$result = & CiTool.exe -lp -json 2>$null | ConvertFrom-Json
$target = @($result.Policies | Where-Object { $_.PolicyID -eq $expectedRetirePolicyId })
if ($result.OperationResult -ne 0 -or $target.Count -ne 1) { throw 'Retirement target is not unique.' }
if ($target[0].BasePolicyID -ne $basePolicyIdText -or $target[0].FriendlyName -ne $expectedRetireFriendlyName) { throw 'Retirement target is not the canonical V10.6.2 Bootstrap policy.' }
...
$replacement = @($result.Policies | Where-Object { $_.PolicyID -eq $ReplacementPolicyId -and $_.BasePolicyID -eq $basePolicyIdText -and $_.FriendlyName -eq $replacementFriendlyName -and $_.IsOnDisk -eq $true -and $_.IsEnforced -eq $true -and $_.IsAuthorized -eq $true })
...
if ($after.OperationResult -ne 0 -or @($after.Policies | Where-Object { $_.PolicyID -eq $expectedRetirePolicyId }).Count -ne 0) { throw 'V10.6.2 policy remains after retirement.' }
...
$replacementAfter = @($after.Policies | Where-Object { $_.PolicyID -eq $ReplacementPolicyId -and $_.BasePolicyID -eq $basePolicyIdText -and $_.FriendlyName -eq $replacementFriendlyName -and $_.IsOnDisk -eq $true -and $_.IsEnforced -eq $true -and $_.IsAuthorized -eq $true })
