...
[CmdletBinding()]
param([string]$CipPath = '', [string]$ExpectedCipSha256 = '')
...
if ([string]::IsNullOrWhiteSpace($CipPath) -or [string]::IsNullOrWhiteSpace($ExpectedCipSha256)) { throw 'CipPath and ExpectedCipSha256 are required; this script never prompts.' }
...
$deployed = @($after.Policies | Where-Object { $_.BasePolicyID -eq $basePolicyIdText -and $_.FriendlyName -eq $friendlyName -and $_.IsOnDisk -eq $true -and $_.IsEnforced -eq $true -and $_.IsAuthorized -eq $true })
if ($after.OperationResult -ne 0 -or $deployed.Count -ne 1) { throw 'V10.6.4 Bootstrap was not uniquely active after deployment.' }
Write-Host "DEPLOYMENT COMPLETE: $($deployed[0].PolicyID)"
