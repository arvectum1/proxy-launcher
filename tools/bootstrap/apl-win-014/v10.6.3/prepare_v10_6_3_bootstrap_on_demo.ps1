<#
.SYNOPSIS
    Author V10.6.3 Bootstrap supplemental policy on ARVECTUM-DEMO (authoring only, CLM-safe).
.DESCRIPTION
    Generates a narrow BootstrapHash supplemental App Control policy for the exact
    0.2.3 candidate. Does NOT deploy. Produces XML + CIP + evidence only.
...
$PolicyFriendlyName = 'Arvectum APL-WIN-014 Harness V10.6.3 Bootstrap'
...
foreach ($entry in @(
    ...
    if ($actual -ine $entry.Expected) { throw "Hash mismatch for $($entry.Label): expected $($entry.Expected), got $actual" }
    Write-Host "  $($entry.Label): PASS ($actual)"
}

$timestamp = Get-Date -Format 'yyyy-MM-ddTHH:mm:ss.ffffffZ'
$outDir = Join-Path $scriptDir "v10.6.3-authoring-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
New-Item -ItemType Directory -Path $outDir -Force | Out-Null

$stagingDir = Join-Path $outDir 'scan'
New-Item -ItemType Directory -Path $stagingDir -Force | Out-Null
Copy-Item -LiteralPath $setupPath -Destination (Join-Path $stagingDir $hashes.files.setup_exe.filename) -Force
Copy-Item -LiteralPath $appPath   -Destination (Join-Path $stagingDir 'Arvectum Proxy Launcher.exe') -Force

Write-Host ''
Write-Host '=== Generating V10.6.3 Bootstrap hash policy ==='
$policyXml = Join-Path $outDir 'Arvectum-APL-WIN-014-V10.6.3-Bootstrap.xml'
...
Write-Host ''
Write-Host '=== V10.6.3 Bootstrap hash rules ==='
$hashRuleIds = @()
$hashRuleNames = @()
$hashRuleHashes = @()
...
$evidence = New-Object PSObject -Property @{
    schema = 'arvectum.proxy.apl-win-014-v10.6.3-bootstrap-authoring.v1'
    task = 'APL-WIN-014'
...
Write-Host ''
Write-Host '=== V10.6.3 Bootstrap authoring: COMPLETE ==='
Write-Host "Output: $outDir"
Write-Host "Deployment: NOT PERFORMED"
Write-Host 'DO NOT deploy until install_v10_6_3_bootstrap_on_demo.ps1 is run manually.'
